...
async def sync(conn) -> dict:
    ...
    try:
        ...
        sq_env = SquareEnvironment.PRODUCTION if SQUARE_ENV == "production" else SquareEnvironment.SANDBOX
        client = Square(token=token, environment=sq_env)

        # ── Catalog items (SDK v44: catalog.list returns SyncPager) ─────
        try:
            for obj in client.catalog.list(types='ITEM').items:
                ...
                for var in variations:
                    ...
                    price = None
                    if hasattr(var_data, 'price_money') and var_data.price_money:
                        price = var_data.price_money.amount
                    name = f"{item_data.name or ''} — {getattr(var_data, 'name', None) or 'Default'}".strip(" —")
                    conn.execute(
                        ...
                        (var.id, name, item_data.description,
                         getattr(var_data, 'sku', None), price,
        ...
        # ── Inventory counts (SDK v44: inventory.batch_get_counts) ───────
        try:
            items = conn.execute("SELECT square_id FROM square_catalog_items").fetchall()
            item_ids = [r["square_id"] for r in items]
            if item_ids:
                for count in client.inventory.batch_get_counts(catalog_object_ids=item_ids[:500]).items:
                    conn.execute(
                        ...
                        (count.catalog_object_id, count.location_id,
                         ...
                         count.calculated_at, datetime.now(timezone.utc).isoformat())
                    )
        except Exception as e:
        ...
        try:
            ...
            for order in (result.orders or []):
                ...
                cust_name = ftype = fstate = None
                if order.fulfillments:
                    f = order.fulfillments[0]
                    ftype = f.type
                    fstate = f.state
                    if hasattr(f, 'shipment_details') and f.shipment_details and f.shipment_details.recipient:
                        cust_name = f.shipment_details.recipient.display_name
                    elif hasattr(f, 'pickup_details') and f.pickup_details and f.pickup_details.recipient:
                ...
                for li in (order.line_items or []):
                    ...
                    conn.execute(
                        """INSERT OR IGNORE INTO square_order_items
                           ...
                           VALUES (?,?,?,?,?,?)""",
                        (order.id, getattr(li, 'catalog_object_id', None), li.name,
